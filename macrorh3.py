...
class MacroApp:
    def __init__(self, root):
        ...
        # Combobox para selecionar a tecla
        ...
        self.loop_interval_entry.insert(0, "20")  # Valor padrão para o loop interval
    ...
    def iniciar_macro(self):
        """Inicia a macro e registra a hotkey. O loop só começará quando o usuário pressionar a tecla pela primeira vez."""
        if self.running:
            return
        self.running = True
        self.start_button.config(state=tk.DISABLED)
        self.stop_button.config(state=tk.NORMAL)
        self.loop_started = False  # Reseta a flag para que o loop inicie na primeira pressão

        # Obtém a tecla selecionada
    ...
    def ativar_macro(self, tecla, mensagens, flood_speed):
        """
        Registra a hotkey. Se o loop estiver ativado, na primeira pressão da tecla o loop é iniciado;
    ...
    def loop_mensagens(self, mensagens, flood_speed, loop_interval):
        """
        Loop: envia as mensagens com a cadência (flood_speed) e espera o loop_interval para repetir.
    ...
    def parar_macro(self):
        """Para a macro e desabilita os hotkeys."""
        if not self.running:
            return
        self.running = False
